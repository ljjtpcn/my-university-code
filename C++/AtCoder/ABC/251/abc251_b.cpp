...
// Contest 251) URL: https://atcoder.jp/contests/abc251/tasks/abc251_b Memory
...
void solve() {
    ...
    vector<int> good((int)3e6 + 10, 0);

    for (int i = 0; i < n; i++) good[arr[i]] = 1;
    
    for (int i = 0; i < n; i++)
        for (int j = i + 1; j < n; j++) good[arr[i] + arr[j]] = 1;

    for (int i = 0; i < n; i++)
        for (int j = i + 1; j < n; j++)
            for (int k = j + 1; k < n; k++) good[arr[i] + arr[j] + arr[k]] = 1;
    cout << accumulate(good.begin() + 1, good.begin() + M + 1, 0);
}
