...
// Contest URL: https://codeforces.com/gym/103145/problem/J Memory Limit: 256 MB
// Time Limit: 4000 ms
#include <bits/stdc++.h>
#include <cmath>
...
// 1.13、矢量夹角角度
double Angle(const Point& op, const Point& p1, const Point& p2,
             bool is_radian = true);
...
// 2.4、二阶贝塞尔曲线插值
vector<Point> secondOrderBezier(const Point& s, const Point& e, const Point& p,
                                int inter_num);

// 2.5、三阶贝塞尔曲线插值
vector<Point> thirdOrderBezier(const Point& s, const Point& e, const Point& p1,
                               const Point& p2, int inter_num);
...
// 4.7、判断圆是否在多边形内部
bool isCircleInPolygon(const vector<Point>& polygon, const Point& p,
                       double radius);
...
// 4.12、计算多边形的范围
void boxOfPolygon(const vector<Point>& polygon, Point& down_left,
                  Point& up_right);
...
// 5.4、两圆之间的关系
int circleToCircle(const Point& c1, double raduis1, const Point& c2,
                   double radius2);
...
//
double dotMultiply(const Point& op, const Point& p1, const Point& p2) {
    return ((p1.x - op.x) * (p2.x - op.x) + (p1.y - op.y) * (p2.y - op.y) +
            (p1.z - op.z) * (p2.z - op.z));
}
...
bool isponl(const Point& p, const Line& l) {
    ...
    // 点是否在线段上
    if (l.is_seg) {
        if (equal(p, l.s) || equal(p, l.e)) return true;
...
bool isSegIntersect(const Line& l1, const Line& l2, Point& inter_p) {
    ...
    if (l1.is_seg) {
        ...
        // 判断线段是否相互跨立
        double dot1 = dotMultiply(multiply(sub(l2.s, l1.s), line1),
                                  multiply(sub(l2.e, l1.s), line1));
        double dot2 = dotMultiply(multiply(sub(l1.s, l2.s), line2),
                                  multiply(sub(l1.e, l2.s), line2));
        if (dot1 < 0.0 && dot2 < 0.0) {
            double t1 = length(multiply(sub(l1.s, l2.s), norm2)) /
                        length(multiply(norm2, norm1));
            double t2 = length(multiply(sub(l2.s, l1.s), norm1)) /
                        length(multiply(norm1, norm2));
    ...
    // 直线相交
        ...
        if (Cos(line1, line2) == 1.0) return false;
...
// 2.5、三阶贝塞尔曲线插值
//
// 参数： s： 起点 e : 终点 p1，p2 : 控制点
// inter_num：插值点数量（不包括起始点） 返回值包括起始点
//
vector<Point> thirdOrderBezier(const Point& s, const Point& e, const Point& p1,
                               const Point& p2, int inter_num) {
    vector<Point> res;
    ...
    for (int i = 1; i <= inter_num; ++i) {
        ...
        res.push_back(
            add(add(add(mul(s, a4), mul(p1, a3)), mul(p2, a2)), mul(e, a1)));
    }
    res.push_back(e);
...
//
bool isPointInTriangle(const Triangle& t, const Point& p, double& u,
                       double& v) {
...
Point ltotInterPoint(const Triangle& t, const Line& l) {
    ...
    double ratio = dotMultiply(point_vec, unit_plane_normal) /
                   dotMultiply(unit_plane_normal, line_vec);
...
void checkConvex(const vector<Point>& polygon, vector<bool>& flags) {
    ...
    for (int i = 1; i < polygon.size(); ++i) {
        if (polygon[i].y < polygon[index].y ||
            (polygon[i].y == polygon[index].y &&
             polygon[i].x < polygon[index].x)) {
            index = i;
        }
    }
    /* 判断每个点的凹凸性
    ...
    flags[index] = true;
    while (size) {
        if (multiply(polygon[index], polygon[(index + 1) % size],
...
double areaOfPolygon(const vector<Point>& polygon) {
    ...
    for (int i = 0; i < size; ++i) {
        area += polygon[i].y *
                (polygon[(i - 1 + size) % size].x - polygon[(i + 1) % size].x);
    }
...
bool isPointInPolygon(const vector<Point>& polygon, const Point& p) {
    ...
    for (int i = 0; i < polygon.size(); ++i) {
        ...
        Line seg(s, e, true);
        Point inter_p;
        if (isSegIntersect(p_line, seg, inter_p)) { cnt++; }
    }
...
bool isSegInPolygon(const vector<Point>& polygon, const Line& l) {
    ...
    if (is_s_in && is_e_in) {
        for (int i = 0; i < polygon.size(); ++i) {
            ...
            Line seg(s, e, true);
            Point inter_p;
            if (isSegIntersect(l, seg, inter_p)) { return false; }
        }
        return true;
    } else {
        return false;
...
//
bool isCircleInPolygon(const vector<Point>& polygon, const Point& c,
                       double radius) {
    for (int i = 0; i < polygon.size(); ++i) {
        const Point& p1 = polygon[i];
        const Point& p2 = polygon[(i + 1) % polygon.size()];
        Line line(p1, p2, true);
        if (segToCircle(c, radius, line) != 2) return false;
    }
    return true;
}
...
vector<Point> findConvexGraham(const vector<Point>& points) {
    ...
    for (auto iter = (++cos_map.begin()); iter != cos_map.end(); ++iter) {
        ...
        if (multiply(vec1, vec2).z >= 0) result_stack.push(first);
...
vector<Point> findConvex(const vector<Point>& points) {
    ...
    for (int i = 2; i < tmp_points.size(); ++i) {
        ...
        while (upper_hull.size() > 2 &&
               multiply(sub(upper_hull[upper_hull.size() - 2],
                            upper_hull[upper_hull.size() - 3]),
                        sub(upper_hull[upper_hull.size() - 1],
                            upper_hull[upper_hull.size() - 3]))
                       .z >= 0) {
    ...
    for (int i = tmp_points.size() - 3; i >= 0; --i) {
        ...
        while (lower_hull.size() > 2 &&
               multiply(sub(lower_hull[lower_hull.size() - 2],
                            lower_hull[lower_hull.size() - 3]),
                        sub(lower_hull[lower_hull.size() - 1],
                            lower_hull[lower_hull.size() - 3]))
                       .z >= 0) {
...
Point pointInPolygon(const vector<Point>& polygon) {
    ...
    Point min_p;
    for (int i = 0; i < polygon.size(); ++i) {
        if (i == index || i == ((index - 1 + size) % size) ||
            i == ((index + 1) % size))
            continue;
        flag = true;
    ...
    if (!flag) { return div(add(pre_p, next_p), 2); }
    // 返回最近点和凸顶点的中点
...
void solve() {
    ...
    double r1 = acos(-1) * 1.0 / (180.0 / r);
    Point ans1, ans2;
    ans1.x = u * cos(r1) + (y * w - z * v) * sin(r1) +
             x * (x * u + y * v + z * w) * (1.0 - cos(r1));
    ans1.y = v * cos(r1) + (z * u - x * w) * sin(r1) +
             y * (x * u + y * v + z * w) * (1.0 - cos(r1));
    ans1.z = w * cos(r1) + (x * v - y * u) * sin(r1) +
             z * (x * u + y * v + z * w) * (1.0 - cos(r1));

    double r2 = PI * 1.0 / (180.0 / (360 - r));
    ans2.x = u * cos(r2) + (y * w - z * v) * sin(r2) +
             x * (x * u + y * v + z * w) * (1.0 - cos(r2));
    ans2.y = v * cos(r2) + (z * u - x * w) * sin(r2) +
             y * (x * u + y * v + z * w) * (1.0 - cos(r2));
    ans2.z = w * cos(r2) + (x * v - y * u) * sin(r2) +
             z * (x * u + y * v + z * w) * (1.0 - cos(r2));

    if (ans1.z > ans2.z)
        cout << ans1.x << ' ' << ans1.y << ' ' << ans1.z << endl;
    else
        cout << ans2.x << ' ' << ans2.y << ' ' << ans2.z << endl;
}
